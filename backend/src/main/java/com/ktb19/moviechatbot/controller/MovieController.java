...
import com.ktb19.moviechatbot.dto.ParsedQueryResponse;
...
public class MovieController {
     ...
     */
    @Operation(summary = "메시지 파싱", description = "메시지를 파싱하여 영화 이름, 지역, 날짜, 시간을 추출")
    ...
    /**
     * 이전에 이미 파싱된 객체와 부족한 정보에 대한 사용자의 메시지를 같이 받아, 완성된 영화 이름, 지역, 날짜, 시간이 포함된 객체를 반환합니다.
     *
     * @param request 이전에 파싱된 결과와 부족한 정보에 대한 사용자의 추가 메세지를 담고 있는 AdditionalQueryRequest 객체
     * @return 파싱된 쿼리 결과를 포함한 ResponseEntity 객체
     */
    @Operation(summary = "추가 메세지 파싱", description = "이전에 이미 파싱된 객체와 부족한 정보에 대한 사용자의 메시지를 같이 받아, 완성된 영화 이름, 지역, 날짜, 시간이 포함된 객체를 반환")
     ...
     */
    @Operation(summary = "영화 상영 시간 조회", description = "파싱된 영화 정보를 기반으로 상영 시간 조회")
